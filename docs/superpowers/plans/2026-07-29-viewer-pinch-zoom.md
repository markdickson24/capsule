...
- **Known approximation:** pan bounds use `SCREEN_WIDTH`/`SCREEN_HEIGHT`, but the photo renders with `contentFit="contain"` and is letterboxed on one axis. On that axis the bound is therefore slightly generous, so a fully-panned image can show a sliver of black beyond its edge. Computing exact bounds needs the image's intrinsic aspect ratio (available from expo-image's `onLoad`); deliberately not done, as the visible effect is small and it would add per-item state.
