...
- **`KeyboardAvoidingView` is unreliable inside a React Native `<Modal>`** — same root cause: the modal is a separate window, so the KAV measures its frame origin wrong and often doesn't lift the input above the keyboard (an input pinned `position:absolute bottom:0` stays *under* the keyboard). For a text field inside a modal, **track the keyboard height yourself** via `Keyboard.addListener('keyboardWillShow'/'keyboardWillHide', …)` (use `keyboardDidShow/Hide` on Android) and apply it as `paddingBottom`/offset to a full-height, `justifyContent:'flex-end'` overlay. This is what `CapsuleDetailScreen`'s `MediaViewerModal` caption editor does (`kbHeight` state). `PreviewScreen`'s caption is NOT in a modal (it's a screen), so its outer-`SafeAreaView`/inner-`KeyboardAvoidingView` pattern works fine there.
