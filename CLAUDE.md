...
| `capsule_members` | id, capsule_id, user_id, role (owner/contributor/viewer), invited_at, joined_at (null = pending), checkin_lat, checkin_lng, checkin_at, contribution_nudge_7d_sent_at / _3d_sent_at / _1d_sent_at (nullable — per-tier dedupe stamps, see "Contribution Nudges") |
...
- **Two route shapes** (discriminated at runtime in a `useMemo`):
  ...
  - `{ media: PendingMedia[], source?: 'share' | 'camera'; targetCapsuleId?: string }` — multi-item form, used by `useShareIntent` and `CapsuleDetailScreen`'s "+ Add Photos" (library/camera picks route through here too — see below — so they get per-item captions and the same resize pipeline as camera/share uploads, instead of enqueuing directly with neither)
...
**Upload flow:** all media uploads (Add Photos picker, camera, and everything arriving from PreviewScreen) go through the **background upload queue** (`src/lib/uploadQueue.ts`). The "+ Add Photos" picker (`pickFromLibrary`/`pickFromCamera`) doesn't enqueue directly — `goToPreview()` hands the picked assets to `Preview` (`{ media, source: 'camera', targetCapsuleId: capsuleId }`) so they get per-item captions and the shared resize pipeline before Preview itself enqueues them; `useUploadTasks(capsuleId)` renders the queue as local-URI **pending tiles** above the photo grid (spinner overlay while uploading; failed tiles get Retry + dismiss). A surprise-mode locked box shows an "N uploading…" line instead of tiles. An effect watches the task count and calls `fetchPhotos()` as each task lands (the queue has already invalidated `media:`/`signedUrls:`/`capsule:` for the capsule). The aggregate "Uploading n/N" row + `ProgressBar` is driven by `uploadQueue.getProgress(capsuleId)`.
...
- **Archive / restore capsule** — sets/clears `archived_at`. Hides from main feed; appears in collapsible "Archived" section on Home with a Restore button. **Any joined member can archive or restore, not just the owner** — `CapsuleDetailScreen` gates the button on `canArchive = isOwner || myMember?.joined_at != null` (a pending, not-yet-accepted invitee cannot). Because the blanket `capsules` UPDATE RLS policy is owner-only and covers every column (no column-level exception), this goes through a dedicated `security definer` RPC, **`set_capsule_archived(p_capsule_id, p_archived)`** (`20260706120000_member_archive_capsule.sql`), rather than a direct `.update()` — it authorizes inline (owner OR a `capsule_members` row with `joined_at is not null`) and only ever touches `archived_at`. `CapsuleDetailScreen`'s toggle, `HomeScreen.restoreCapsule`, and `HomeScreen.archiveCapsule` (see below) all call this RPC. `HomeScreen`'s `archivedCapsules` list is derived from `allCapsules`, which is already scoped to joined membership (the underlying query filters `joined_at is not null`) — so no extra owner filter is needed there; any archived (or, symmetrically, any active) capsule a member can see is one they're allowed to restore/archive. **Archive is reversible and lives in its own section, separate from Danger Zone** (which is Delete-only, `isOwner`-gated) — since archive/restore is a two-way toggle, not a destructive action. `CapsuleDetailScreen`'s archive button uses `navigation.goBack()` (not `navigation.reset`, which used to wipe the whole nav stack silently) and, on success, shows a toast with an **Undo** action (`toast.show(message, { label, onPress })` — see "Toast" below) that re-calls the same RPC in reverse. `EditCapsuleScreen`'s own Archive button (that screen is still owner-only end-to-end, gated by its `loadCapsule` early-return) was left as a direct `.update()` — unreachable by non-owners anyway since the whole screen bounces them.
...
- **Delete capsule** — clears storage files from `capsule-media` bucket first, then deletes the capsule row (cascades to members, media, reactions, notifications). Confirmation required via `<ConfirmModal>` (not `Alert.alert` — that no-ops on web). Available from EditCapsule and CapsuleDetail Danger Zone (Delete only — see Archive above). Owner-only.
...
**Schema:** `groups` (id, name, `created_by` → `users(id)` **on delete cascade**, `recurrence_interval` [`weekly`/`monthly`/`yearly`/`manual`], `unlock_duration_hours`, `next_capsule_at`, `last_capsule_at`, `created_at`); `group_members` (`group_id`, `user_id`, `joined_at`, unique on the pair); `capsules.group_id` — nullable FK, set on any capsule (manual or cron-created) that originated from a group.
...
**Manual create respects the schedule.** When a member manually starts a capsule for a group that has a real recurrence (not `manual`), `CreateScreen`'s group branch bumps `next_capsule_at` forward from now (via `updateGroup(groupId, { recurrence: group.recurrence_interval })`, which recomputes the date as a side effect of "updating" the same recurrence) right after creating the capsule — otherwise the cron would still fire on the old schedule and double up. Best-effort/non-fatal: worst case on failure is one extra capsule next cycle, not worth surfacing to the user over their own "Lock Capsule" tap. The capsule itself is owned by whoever tapped create, not necessarily the group's creator — an accepted asymmetry, not a bug.

**`ManageGroupScreen`** (`src/screens/app/ManageGroupScreen.tsx`, route `ManageGroup { groupId }`, `animation: 'slide_from_bottom'`) — creator-only (bounces non-creators via `goBack()`), reached from a gear icon in `GroupDetailScreen`'s header (creator only, alongside the existing trash icon in `styles.headerActions`). Rename, change recurrence + unlock-duration (same chip components as `CreateGroupScreen`), and a member list with debounced add-member search (excludes existing members and `blockStore.has(id)`, matching `InviteModal`'s pattern) and remove (trash icon → `ConfirmModal`; the creator can't be removed). Wires up `updateGroup` / `addGroupMember` / `removeGroupMember` (`src/lib/groups.ts`), previously dead code with no UI caller. Invalidates `groups`/`group:${id}`/`group-members:${id}` on any mutation. Newly-added members only appear in *future* group capsules — existing active ones are not backfilled.
...
**Ownership safety net.** `groups.created_by` is `on delete cascade` to `users(id)` — deleting a group's creator used to silently delete the group for every other member too, with no warning, even for a group with years of history and an active recurring schedule (`delete_my_account` predates groups and never accounted for them). Fixed in `delete_my_account` (`20260710030000_delete_account_group_transfer.sql`): immediately before the final `auth.users` delete, every group the caller created is reassigned to another existing member (oldest-joined, deterministic) if one exists. A group with no other member needs no special handling — it still cascade-deletes exactly as before, since "delete only if they were the last one" falls out of the existing FK once the transfer-if-possible case is handled first. `SettingsScreen`'s delete-account copy (both the inline `Account` section helper text and the confirmation sheet body) mentions that owned groups are handed off rather than destroyed.
