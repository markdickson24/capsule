...
- Registers Expo push token (native only) and stores in `users.push_token` — but **only if permission is already granted**. `registerToken` never calls `requestPermissionsAsync`; the native prompt fires exactly once, from `requestPushPermission(userId)` (exported from the same module, no-op `false` on web). Two call sites use it, both contextual: the Onboarding "Don't miss it" primer (step 4, after the first capsule is created) and the standalone-create **seal ceremony** (see "Seal Ceremony" below). A "maybe later" answer in onboarding writes `cap_notif_reprime:<userId>` to AsyncStorage. `getPushPermissionStatus()` (same module; `'granted'|'denied'|'undetermined'|'unavailable'`, `'unavailable'` on web) is the read-only status check a primer uses to decide whether to show a soft ask at all — it never prompts.
...
**Post-create invite nudge** — when `route.params.justCreated` is true (set by `CreateScreen` on navigation right after a capsule is made) and `members.length === 1`, a dismissible callout ("Invite people — capsules are better together") renders above the Media section, with an Invite button opening the same `InviteModal`. A single-member capsule is a failed core loop (no reveal, no award voting to anticipate), so this is the one moment worth nudging; it's gone once another member joins or the user dismisses it, and never reappears on a later visit (the param isn't persisted). The `justCreated` navigation happens *after* the seal ceremony (below), not immediately at create time.

**Seal Ceremony (`src/components/SealCeremony.tsx`)** — the app is named after *sealing something away*; the unlock gets a reveal animation, so the lock gets its own moment. After a standalone capsule create, `CreateScreen.handleCreate` sets a `sealed` state instead of navigating, which renders `<SealCeremony>` full-screen (a `Modal`, so it covers the tab bar): an accent lock scales in (spring) with `haptics.success()`, "Sealed." + a subtitle (`opens {date}`, or `opens when you're all together` for proximity), held ~1.5s. Then, **only when push permission is `'undetermined'` and a one-time `cap_seal_primer_seen` flag is unset**, it shows a soft primer ("Know the second it unlocks?" → Notify me / Not now) *before* the OS dialog — reusing `requestPushPermission` / `getPushPermissionStatus`. This naturally catches only users who skipped/deferred the onboarding primer, once. Its `onDone` navigates to `CapsuleDetail` with `justCreated: true` (the invite nudge above). Onboarding has its **own** inline step-5 seal ceremony (`OnboardingScreen`) — intentionally not shared with this component this round (lower risk), so the two are duplicated.
