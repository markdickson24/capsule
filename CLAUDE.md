...
**Community section** — three `SettingsRow`s (`Linking.openURL`) to the Discord invite (`DISCORD_URL`), Instagram (`INSTAGRAM_URL`), and TikTok (`TIKTOK_URL`), using `logo-discord`/`logo-instagram`/`logo-tiktok` Ionicons. URLs are constants at the top of `SettingsScreen.tsx` — update them there if the handles change.
