...
**Why there's no server component.** The countdown is rendered by **iOS itself** from the activity's `staleDate` (set to the deadline at `start()`) — once started, a Live Activity needs **zero** updates to keep ticking correctly, including while the device is offline. The only things `update()` ever changes are `photoCount`/`memberCount`, and only while the app is actually running — there is no cron, no edge function, no push-to-start for this feature.
...
A `runningRef` guard prevents two overlapping reconcile passes (e.g. a foreground event landing mid-mount-pass) from both racing the native module's unserialized check-then-act in `start()` and creating a duplicate card. A `generationRef` guard additionally protects against an *abandoned* pass (one that hit the 30s `withTimeout` and gave up) resuming after a newer pass has already taken over — both guards must stay; removing either reintroduces the duplicate-activity bug they were added to fix.
...
- **`photoCount` goes stale between app foregrounds** — the accepted cost of having no server component; there's nothing pushing an updated count to a card while the app isn't running.
- **Turning the toggle ON has no immediate effect** — the card only appears at the *next* reconcile pass (next foreground). Turning it OFF is instant (`CapsuleDetailScreen.handleToggleLiveActivity` calls `endLiveActivity(capsuleId, true)` directly, not waiting for a reconcile).
