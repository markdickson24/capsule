...
- **Front camera photos are mirrored after resize:** `processPhoto(uri, mirror)` in `CameraScreen.tsx` runs `resizeForUpload()` first, then — only when `mirror` is true — a second `expo-image-manipulator` pass with `flip: FlipType.Horizontal`. `takePhoto()` passes `mirror: facing === 'front'`. This is single-camera-photo only: `CameraView`'s `mirror` prop only affects the live preview, not the saved JPEG, so without this step front-camera stills come out un-mirrored relative to what the user saw. Dual-camera composites are already mirrored natively (`modules/expo-dual-camera`, front lens mirrored at the AVFoundation capture connection) and videos are untouched — this flip is photos-only.
